...
int points_to_boxes_dist_gpu(at::Tensor boxes_tensor, at::Tensor pts_tensor, at::Tensor pts_dist_tensor) {
    // params boxes: (B, N, 7) [x, y, z, dx, dy, dz, heading], (x, y, z) is the box center, each box DO NOT overlaps
    // params pts: (B, num_points, 3) [x, y, z]
    // params pts_dist: (B, num_points)
    int batch_size = boxes_tensor.size(0);
    int boxes_num = boxes_tensor.size(1);
    int pts_num = pts_tensor.size(1);
    ...
    const float *boxes = boxes_tensor.data<float>();
    const float *pts = pts_tensor.data<float>();
    float *pts_dist = pts_dist_tensor.data<float>();

    points_to_boxes_dist_launcher(batch_size, boxes_num, pts_num, boxes, pts, pts_dist);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("points_to_boxes_dist_gpu", &points_to_boxes_dist_gpu, "points_to_boxes_dist_gpu forward (CUDA)");
}
