...
__device__ inline float calc_pt_to_box3d_dist(const float *pt, const float *box3d, float &local_x, float &local_y){
    // param pt: (x, y, z)
    // param box3d: [x, y, z, dx, dy, dz, heading] (x, y, z) is the box center
    ...
    float z_dist = fabsf(z - cz) - dz / 2.0;
    if (z_dist < MARGIN) z_dist = 0.0;

    lidar_to_local_coords(x - cx, y - cy, rz, local_x, local_y);
    float x_dist = fabsf(local_x) - dx / 2.0, y_dist = fabsf(local_y) - dy / 2.0;
    if (x_dist < MARGIN) x_dist = 0.0;
    if (y_dist < MARGIN) y_dist = 0.0;
...
__global__ void points_to_boxes_dist_kernel(int batch_size, int boxes_num, int pts_num, const float *boxes,
    const float *pts, float *pts_dist){
    // params boxes: (B, N, 7) [x, y, z, dx, dy, dz, heading] (x, y, z) is the box center
    // params pts: (B, npoints, 3) [x, y, z]
    // params pts_dist: (B, npoints), 0 if point inside box
    ...
    boxes += bs_idx * boxes_num * 7;
    pts += bs_idx * pts_num * 3 + pt_idx * 3;
    pts_dist += bs_idx * pts_num + pt_idx;

    float local_x = 0, local_y = 0;
    float min_dist = 1e5;
    for (int k = 0; k < boxes_num; k++){
        float cur_dist = calc_pt_to_box3d_dist(pts, boxes + k * 7, local_x, local_y);
...
void points_to_boxes_dist_launcher(int batch_size, int boxes_num, int pts_num, const float *boxes,
    const float *pts, float *pts_dist){
    // params boxes: (B, N, 7) [x, y, z, dx, dy, dz, heading] (x, y, z) is the box center
    // params pts: (B, npoints, 3) [x, y, z]
    // params pts_dist: (B, npoints), 0 if point inside box
    ...
    dim3 blocks(DIVUP(pts_num, THREADS_PER_BLOCK), batch_size);
    dim3 threads(THREADS_PER_BLOCK);
    points_to_boxes_dist_kernel<<<blocks, threads>>>(batch_size, boxes_num, pts_num, boxes, pts, pts_dist);
